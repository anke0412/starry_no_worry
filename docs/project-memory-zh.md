...
- 双体比较：
  - A + B → 比较盘
- 双体衍生：
  - Phase 4 起不再保留旧 `关系流年盘` 路线；双人推运改为在批准的基础双人盘之上继续做派生
- 双体融合：
  - A + B → 组合盘 / Davison / 马盘
...
- `马克思盘` 的稳定实现 contract 已确认：
  - 不做单一融合盘
  - 先生成共享 `davisonChart`
  - 再分别生成 `primaryMarxChart` 与 `secondaryMarxChart`
  - 顶层 `chartType` 仍然是 `marx`，但真实盘面数据放在这两个视角盘里
- 这些双人 `次限/三限` 的统一口径是：
  - 先生成基础双人盘
  - 再对该基础盘做对应的 `次限` 或 `三限`
  - 前端入口统一收纳到 `流年推运盘`
- 其中 `组合盘`、`时空中点盘`、`马克思盘` 的双人 `次限/三限` 已经源码支持：
  - 返回结构统一为基础关系盘 + 推运盘 + overlay
  - `relatedCharts` 会同时保留双方本命盘与基础关系盘，方便后续 AI / 联动解读继续复用
  - `马克思盘` 的推运继续保留双视角 contract：分别保留 `primary` / `secondary` 视角的基础盘、推运盘与 overlay
