...
from app.models.chart import BirthProfile, LunarReturnChartRequest, SolarReturnChartRequest
...
def test_find_lunar_return_datetime_returns_exact_time_near_anchor():
    anchor = LunarReturnSearchInput(
        natal_profile=build_profile(),
        anchor_date="2026-05-16",
        anchor_time="06:30",
        anchor_timezone="Asia/Seoul",
    )
    result = find_lunar_return_datetime(anchor)
    previous_cycle = find_lunar_return_datetime(
        LunarReturnSearchInput(
            natal_profile=build_profile(),
            anchor_date="2026-04-19",
            anchor_time="06:30",
            anchor_timezone="Asia/Seoul",
        )
    )
    next_cycle = find_lunar_return_datetime(
        LunarReturnSearchInput(
            natal_profile=build_profile(),
    ...
    anchor_local = datetime.fromisoformat("2026-05-16T06:30:00").replace(tzinfo=ZoneInfo("Asia/Seoul"))

    assert isinstance(result, datetime)
    assert local_result.year == 2026
    assert abs((result - anchor_local.astimezone(ZoneInfo("UTC"))).total_seconds()) <= abs((previous_cycle - anchor_local.astimezone(ZoneInfo("UTC"))).total_seconds())
    assert abs((result - anchor_local.astimezone(ZoneInfo("UTC"))).total_seconds()) <= abs((next_cycle - anchor_local.astimezone(ZoneInfo("UTC"))).total_seconds())
