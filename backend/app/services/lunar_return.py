...
def bracket_lunar_return(
    ...
    while current < end:
        ...
        if current_offset == 0:
            brackets.append((current, current))
        elif next_offset == 0 or current_offset * next_offset < 0:
            brackets.append((current, next_time))
    ...
    return min(
        brackets,
        key=lambda bracket: abs((((bracket[0] + (bracket[1] - bracket[0]) / 2) - anchor).total_seconds())),
    )
